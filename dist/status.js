...
import { listOpenIssuesByLabel } from "./gh.js";
...
export async function runStatus() {
    ...
    const ready = all.filter(i => i.labels.some(l => l.name === labels.prdReady) &&
        !i.labels.some(l => afkManagedLabels.includes(l.name)));
...
async function section(title, issues, showWorktree, showPr = false) {
    if (issues.length === 0)
        return;
    process.stdout.write(`\n${bold}${title}${reset}\n`);
    for (const i of issues) {
        ...
        if (showPr) {
            ...
            if (pr)
                process.stdout.write(`     ${green}→ ${pr}${reset}\n`);
        }
    }
}
async function commitCount(wt) {
