 ...
 */
export async function mergeBaseIntoWorktree(wt: Worktree): Promise<MergeResult> {
  ...
  const merge = await execa(
    "git",
    ["-C", wt.path, "merge", "--no-edit", "--no-ff", baseRef],
    { reject: false, env: { ...process.env, GIT_EDITOR: "true" } },
  );
  if (merge.exitCode === 0) {
  ...
  const unmerged = await execa(
    "git",
    ["-C", wt.path, "diff", "--name-only", "--diff-filter=U"],
    { reject: false },
  );
