...
import { brief, briefAllAwaitingQa } from "./brief.ts";
import { bootstrap } from "./bootstrap.ts";
import { startDashboard } from "./dashboard/server.ts";
import { test as telegramTest } from "./subscribers/telegram.ts";
...
async function main(): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case "plan": {
      ...
      if (!shouldDispatch || result.ready.length === 0) return;
    ...
    case "brief": {
      const arg = rest[0];
      if (!arg) await briefAllAwaitingQa();
      else {
    ...
    case "reset": {
      ...
      const { resetIssue } = await import("./reset.ts");
...
function printPlan(result: Awaited<ReturnType<typeof plan>>, willDispatch: boolean): void {
  const slotCapped = result.totalReady - result.ready.length;
  process.stdout.write(
    `ready=${result.totalReady} (dispatching ${result.ready.length} this round` +
  ...
  for (const i of result.ready) process.stdout.write(`  → #${i.number} ${i.title}\n`);
  for (const i of result.blocked) process.stdout.write(`  ⏸  #${i.number} ${i.title} (blocked-by)\n`);
  for (const i of result.inFlight) process.stdout.write(`  ▶  #${i.number} ${i.title}\n`);
  for (const i of result.skippedPrds) process.stdout.write(`  ⏭  #${i.number} ${i.title} (PRD — needs /to-issues)\n`);
  for (const i of result.skippedAtCap) process.stdout.write(`  ⛔ #${i.number} ${i.title} (attempt cap reached — needs human)\n`);
...
async function dispatchReady(ready: Awaited<ReturnType<typeof plan>>["ready"]): Promise<void> {
