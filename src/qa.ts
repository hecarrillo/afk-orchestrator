...
import { listOpenIssuesByLabel, addLabels, removeLabels, commentOnIssue, reviewPullRequest, clearAttemptCount } from "./gh.ts";
import { labels, afkManagedLabels } from "./labels.ts";
...
async function handleVerdict(issueNumber: number, prUrl: string | null, verdict: string, rl: ReturnType<typeof createInterface>): Promise<void> {
  if (verdict === "a") {
    if (!prUrl) {
...
async function findPrForBranch(branch: string): Promise<string | null> {
  ...
  try {
    const arr = JSON.parse(result.stdout) as { url: string }[];
