...
import { getIssue, type Issue } from "./gh.ts";
...
export async function brief(numberArg: number): Promise<void> {
...
export async function briefAllAwaitingQa(): Promise<void> {
  const { listOpenIssuesByLabel } = await import("./gh.ts");
...
async function resolve(num: number): Promise<[Issue | null, PR | null]> {
...
async function fetchPr(num: number): Promise<PR | null> {
...
async function prForBranch(branch: string): Promise<PR | null> {
  ...
  try {
    const arr = JSON.parse(list.stdout) as { number: number }[];
...
function parseLinkedIssue(pr: PR): number | null {
  // Branch convention: afk/issue-N
  const branchMatch = pr.headRefName.match(/issue-(\d+)/);
  if (branchMatch && branchMatch[1]) return Number(branchMatch[1]);
  // PR body: "closes #N" / "resolves #N" (case-insensitive)
  const bodyMatch = pr.body.match(/(?:closes|resolves|fixes)\s+#(\d+)/i);
  if (bodyMatch && bodyMatch[1]) return Number(bodyMatch[1]);
  return null;
}

function printUnified(issue: Issue | null, pr: PR | null): void {
  // ── ISSUE ─────────────────────────────────────────────────────────────
  if (issue) {
    ...
    if (acLines.length > 0) {
      ...
      for (const line of acLines) process.stdout.write(`  ${line}\n`);
    }
    const whatLines = extractSection(issue.body, "What to build");
    if (whatLines.length > 0) {
      process.stdout.write(`\n${bold}What to build:${reset}\n`);
      for (const line of whatLines.slice(0, 30)) process.stdout.write(`  ${line}\n`);
      if (whatLines.length > 30) process.stdout.write(`  ${dim}... (${whatLines.length - 30} more lines — \`gh issue view ${issue.number}\` for full)${reset}\n`);
    }
  } else {
...
function extractReviewerReport(pr: PR | null): string | null {
  if (!pr) return null;
